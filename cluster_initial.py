...
fig.suptitle("Elbow Curve & Silhouette Score  ·  PAL Clustering", fontsize=13,
             color=TEXT, y=1.01, fontweight="bold")
...
bars = ax.bar(labels_for_bar, counts.values,
              color=[CLUSTER_PALETTE[i] for i in counts.index], width=0.65)
...
KEY_FEATURES = [
    ...
    "Region=North America", "Region=Japan", "Region=MNL HUB", "Region=CEB HUB",
    "Farebrand=Economy Supersaver", "Farebrand=Economy Saver",
    "Farebrand=Economy Flex", "Farebrand=Economy Value", "Farebrand=Business Flex",
...
sns.heatmap(
    heat_norm, ax=ax, cmap="YlOrRd", annot=False,
    linewidths=0.4, linecolor=BG,
    xticklabels=pretty_cols, yticklabels=heat_norm.index,
    cbar_kws={"shrink": 0.6, "label": "Normalised Value (0=low, 1=high)"},
)
ax.set_title("Initial KMeans Centroids  ·  Feature Heatmap  (normalised per feature)",
...
for ci in range(K):
    ...
    ax.scatter(X_pca[sample_idx][mask, 0], X_pca[sample_idx][mask, 1],
               c=CLUSTER_PALETTE[ci], alpha=0.35, s=8, label=f"C{ci} {centroid_labels[ci]}")

# centroid markers
for ci in range(K):
    ax.scatter(centroids_pca[ci, 0], centroids_pca[ci, 1],
               c=CLUSTER_PALETTE[ci], s=220, marker="*", edgecolors="white",
               linewidths=0.8, zorder=10)
    ax.annotate(f"C{ci}", (centroids_pca[ci, 0], centroids_pca[ci, 1]),
                textcoords="offset points", xytext=(6, 4),
                fontsize=8, color="white", fontweight="bold")

ax.set(title=f"PCA Projection  ·  KMeans k=10  (PC1={var_exp[0]:.1f}%  PC2={var_exp[1]:.1f}%)",
       xlabel=f"PC1  ({var_exp[0]:.1f}% variance)", ylabel=f"PC2  ({var_exp[1]:.1f}% variance)")
ax.title.set_color(TEXT)
ax.legend(fontsize=7.5, loc="upper right", framealpha=0.3,
...
RADAR_FEATURES = [
    "lead_time", "fare_per_pax", "PAX Count", "cabin_ord",
    "Region=Middle East", "Itinerary Type=Beyonds (INT - DOM)",
    "Ticketing Channel=Traditional Travel Agency", "Ticketing Channel=WEB/APP",
    "Farebrand=Economy Supersaver", "Farebrand=Economy Flex",
]
RADAR_FEATURES = [f for f in RADAR_FEATURES if f in feature_names]
N_RADAR = len(RADAR_FEATURES)
radar_labels = [
    "Lead Time", "Fare/PAX", "PAX Count", "Cabin Class",
    "Middle East", "Beyond Itin", "TTA Chan", "WEB Chan",
    "Eco Supersaver", "Eco Flex",
][:N_RADAR]

# Normalise centroids on these features
radar_raw  = centroids_df[RADAR_FEATURES].values.astype(float)
radar_norm = (radar_raw - radar_raw.min(axis=0)) / (radar_raw.max(axis=0) - radar_raw.min(axis=0) + 1e-9)
...
fig.suptitle("Cluster Centroid Radar Fingerprints  ·  KMeans k=10",
...
for i, ax in enumerate(axes.flat):
    ...
    ax.tick_params(colors=SUBTEXT)
    ax.set_title(f"C{i}  {centroid_labels[i]}", fontsize=8.5, color=CLUSTER_PALETTE[i],
...
fig.suptitle("Centroid Feature Profiles  ·  KMeans k=10  (bar = relative prominence)",
             fontsize=12, color=TEXT, y=1.005, fontweight="bold")
...
for i, ax in enumerate(axes):
    ...
    ax.set_xlim(0, 1)
    ax.set_ylabel(f"C{i}  {centroid_labels[i]}", rotation=0, labelpad=130,
                  fontsize=8, color=CLUSTER_PALETTE[i], fontweight="bold", va="center")
    ax.set_yticks([])
    ...
    for bar, key, val in zip(bars, pf_keys, centroid_profile.iloc[i]):
        if profile_norm.iloc[i][key] >= 0.55:
            ...
            ax.text(profile_norm.iloc[i][key] + 0.01, bar.get_y() + bar.get_height()/2,
                    raw_disp, va="center", fontsize=6.5, color=TEXT)
...
print("="*72)
summary_keys = ["lead_time", "fare_per_pax", "PAX Count", "cabin_ord", "is_dom",
                "Region=Middle East", "Itinerary Type=Beyonds (INT - DOM)",
                "Ticketing Channel=Traditional Travel Agency", "Ticketing Channel=WEB/APP"]
...
summary.columns = ["LT(d)", "Fare/PAX", "PAX", "Cabin", "DOM%",
                   "ME%", "Beyond%", "TTA%", "WEB%"][:len(summary_keys)]
