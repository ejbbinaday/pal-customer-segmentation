...
def assign_segment(df):
    ...
    seg[(df["PAX Count"]>=4) & (df["Ticketing Channel"]=="Traditional Travel Agency")] = "Pilgrimage"
...
df_enc   = pd.get_dummies(labeled[cat_cols].fillna("Unknown"),
...
X_raw = pd.concat([labeled[num_cols].reset_index(drop=True),
...
clf_params = dict(n_estimators=200, max_depth=12, class_weight=class_weights,
...
for name, sampler in resamplers.items():
    ...
    results[name] = dict(f1=f1, precision=prec, recall=rec, f1_weighted=f1_w,
                         f1_per_class=f1_per, penalty_cost=pen_cost,
                         penalty_cost_pct=pen_cost_pct, cm=cm,
                         y_true=y_r, y_pred=y_pred, n=len(y_r))
...
for ax, (name, (X_r, y_r)) in zip(axes.flat, X_resamp.items()):
    ...
    for bar, val in zip(bars, counts.values):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(counts)*0.01,
                f"{val:,}", ha="center", va="bottom", fontsize=6.5, color=SUBTEXT)
    ax.set_title(f"{name}  (n={len(y_r):,})", fontsize=10,
                 color=METHOD_COLORS.get(name, TEXT), fontweight="bold")
    ax.set_xlabel("")
    ax.set_ylabel("Count")
...
fig.suptitle("Confusion Matrices  ·  5-Fold Cross-Validation",
             fontsize=13, color=TEXT, fontweight="bold", y=1.01)
...
for ax, (name, res) in zip(axes.flat, results.items()):
    ...
    sns.heatmap(cm_norm, ax=ax, cmap="Blues", annot=True, fmt=".2f",
                ...
                linewidths=0.3, linecolor=BG, cbar=False,
                annot_kws={"size": 6.5})
    ax.set_title(f"{name}\nF1={res['f1']:.3f}  cost={res['penalty_cost']:,}",
                 fontsize=9, color=METHOD_COLORS.get(name, TEXT), fontweight="bold")
...
metrics_df = pd.DataFrame({
    m: {"F1 Macro": r["f1"], "F1 Weighted": r["f1_weighted"],
...
for i, (method, row) in enumerate(metrics_df.iterrows()):
    ...
    bars = ax.bar(x + offset, row.values, width=w,
                  color=METHOD_COLORS.get(method, ACCENT), label=method, alpha=0.9)

ax.set_xticks(x)
ax.set_xticklabels(metrics_df.columns, fontsize=10)
ax.set_ylim(0, 1.05)
ax.set_ylabel("Score")
ax.set_title("Classification Metrics by Resampling Strategy", fontsize=12,
...
ax1 = axes[0]
bars = ax1.bar(labels, costs, color=colors, width=0.6,
...
ax2 = axes[1]
ax2.bar(labels, costs_pct, color=colors, width=0.6,
        edgecolor=["white" if i == win_idx else BORDER for i in range(len(labels))],
        linewidth=edge_w)
...
fig.suptitle("Asymmetric Misclassification Penalty Cost by Resampling Strategy",
...
fig.suptitle("Decision Boundaries in PCA Space  ·  Random Forest Classifier",
             fontsize=13, color=TEXT, fontweight="bold", y=1.01)
...
for ax, (name, (X_r, y_r)) in zip(axes.flat, X_resamp.items()):
    ...
    clf_viz = RandomForestClassifier(n_estimators=100, max_depth=8,
    ...
    ax.contourf(xx, yy, Z, alpha=0.35, cmap=cmap_discrete,
                levels=np.arange(-0.5, len(class_names) + 0.5, 1))

    # Scatter sample points
    idx_s = np.random.default_rng(42).choice(len(X_pca_base),
                                              size=min(3000, len(X_pca_base)), replace=False)
    for ci, cname in enumerate(class_names):
        mask = y[idx_s] == ci
        ax.scatter(X_pca_base[idx_s][mask, 0], X_pca_base[idx_s][mask, 1],
                   c=SEG_COLORS.get(cname, "#60A5FA"), s=5, alpha=0.6,
                   label=cname if i == 0 else "")

    ax.set_xlim(x_min, x_max)
    ax.set_ylim(y_min, y_max)
    ax.set_title(f"{name}  |  F1={results[name]['f1']:.3f}  cost={results[name]['penalty_cost']:,}",
                 fontsize=9, color=METHOD_COLORS.get(name, TEXT), fontweight="bold")
    ax.set_xlabel("PC1", fontsize=8)
    ax.set_ylabel("PC2", fontsize=8)

# shared legend
legend_patches = [mpatches.Patch(color=SEG_COLORS.get(c, "#60A5FA"), label=c)
                  for c in class_names]
fig.legend(handles=legend_patches, fontsize=7.5, loc="lower center",
           ncol=len(class_names), framealpha=0.3, facecolor=PANEL,
           edgecolor=BORDER, bbox_to_anchor=(0.5, -0.02))
...
fig.suptitle(f"Winner Deep-Dive: {best_method}  ·  Per-Class F1 & Cost Breakdown",
...
ax1.axvline(results[best_method]["f1"], color="white", ls="--", lw=1.2,
...
for bar, val in zip(bars2, cost_delta):
    x_pos = val + (max(cost_delta)-min(cost_delta))*0.01 if val >= 0 else val - (max(cost_delta)-min(cost_delta))*0.01
...
for rank, (m, r) in enumerate(ranked, 1):
    flag = " ★" if rank == 1 else ""
    print(f"{rank:<6}{m:<24}{r['f1']:<12.4f}{r['f1_weighted']:<14.4f}{r['penalty_cost']:<15,}{r['n']:,}{flag}")
print()
