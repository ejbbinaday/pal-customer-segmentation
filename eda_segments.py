...
plt.rcParams.update({"figure.facecolor":"#111827","axes.facecolor":"#1F2937",
                     "axes.edgecolor":"#374151","text.color":"#F9FAFB",
                     "axes.labelcolor":"#F9FAFB","xtick.color":"#9CA3AF",
                     "ytick.color":"#9CA3AF","grid.color":"#374151",
...
ax.set_title("Proposed Segment Assignment — Record Distribution\n"
             "(Priority waterfall: Corporate > OFW > Balikbayan > Pilgrimage > Family > Last-Minute > Bleisure > Nomad > Budget)",
             fontsize=10, pad=12, color="#F9FAFB")
ax.set_xlabel("PNR Count", color="#9CA3AF")
for bar in bars:
    w = bar.get_width()
    pct = w / len(df) * 100
    ax.text(w + 60, bar.get_y() + bar.get_height()/2,
            f"{w:,}  ({pct:.1f}%)", va="center", fontsize=9, color="#F9FAFB")
ax.set_xlim(0, seg_counts.max() * 1.2)
note = ("Note: 'Mabuhay Loyalist' segment not assignable — Loyalty status is 100% null in sample.\n"
...
ax.set_title("Booking Lead Time Distribution by Proposed Segment (0–180 days)",
             fontweight="bold", pad=12, color="#F9FAFB")
ax.set_xlabel("Days Before Departure"); ax.set_ylabel("Density")
ax.legend(fontsize=8, loc="upper right",
          facecolor="#1F2937", edgecolor="#374151", labelcolor="#F9FAFB")
ax.axvline(3,   color="#EF4444", ls=":", lw=1.2, alpha=.5)
ax.axvline(14,  color="#F59E0B", ls=":", lw=1.2, alpha=.5)
ax.axvline(60,  color="#10B981", ls=":", lw=1.2, alpha=.5)
ax.text(3,  ax.get_ylim()[1]*.95, "3d",  color="#EF4444", fontsize=7, ha="center")
ax.text(14, ax.get_ylim()[1]*.95, "14d", color="#F59E0B", fontsize=7, ha="center")
ax.text(60, ax.get_ylim()[1]*.95, "60d", color="#10B981", fontsize=7, ha="center")
...
order = (df[df["segment"].isin(plot_segs)]
         .groupby("segment")["fare_per_pax"].median()
...
ax.set_title("Fare per PAX by Proposed Segment (USD, excl. outliers)",
             fontweight="bold", pad=12, color="#F9FAFB")
ax.set_ylabel("Fare per PAX (USD)")
for i, (seg, data) in enumerate(zip(order, bp_data)):
    med = data.median()
    ax.text(i+1, med+5, f"${med:.0f}", ha="center", fontsize=8, color="white", fontweight="bold")
...
fig.patch.set_facecolor("#111827")
sns.heatmap(ct_pct.round(1), annot=True, fmt=".1f",
            cmap=sns.color_palette("Blues", as_cmap=True),
            ax=ax, linewidths=0.5, linecolor="#111827",
            cbar_kws={"label":"% within segment","shrink":0.7})
ax.set_title("Ticketing Channel Mix by Proposed Segment (%)",
             fontweight="bold", pad=12, color="#F9FAFB")
ax.set_xlabel("Ticketing Channel"); ax.set_ylabel("")
...
fig, ax = plt.subplots(figsize=(13, 7))
fig.patch.set_facecolor("#111827")
sns.heatmap(rt_pct.round(1), annot=True, fmt=".1f",
            cmap=sns.color_palette("YlOrRd", as_cmap=True),
            ax=ax, linewidths=0.5, linecolor="#111827",
            cbar_kws={"label":"% within segment","shrink":0.7})
ax.set_title("Geographic Region Mix by Proposed Segment (%)",
             fontweight="bold", pad=12, color="#F9FAFB")
...
fb_order = ["Economy Supersaver","Economy Saver","Economy Value","Economy Flex","Premium Economy","Business Value","Business Flex","Non-Rev"]
...
fig, ax = plt.subplots(figsize=(13, 7))
fig.patch.set_facecolor("#111827")
sns.heatmap(ft_pct.round(1), annot=True, fmt=".1f",
            cmap=sns.color_palette("Greens", as_cmap=True),
            ax=ax, linewidths=0.5, linecolor="#111827",
            cbar_kws={"label":"% within segment","shrink":0.7})
ax.set_title("Farebrand Mix by Proposed Segment (%)",
             fontweight="bold", pad=12, color="#F9FAFB")
ax.set_xlabel("Farebrand"); ax.set_ylabel("")
...
fig.patch.set_facecolor("#111827")
fig.suptitle("PNR Booking Month Distribution by Segment\n(All flights: January 2025; booking dates span Jan 2024–Jan 2025)",
             fontsize=10, color="#F9FAFB", y=1.01)

for ax, seg in zip(axes.flat, plot_segs_bm):
    sub = df[df["segment"]==seg]["booking_month"].value_counts().reindex(range(1,13), fill_value=0)
    ...
    peak = sub.idxmax()
    ax.axvline(peak, color="white", ls="--", lw=1, alpha=.5)
...
fig, axes = plt.subplots(2, 5, figsize=(16, 7), sharey=False)
fig.patch.set_facecolor("#111827")
fig.suptitle("PAX Count Distribution by Segment  (clipped at 10+)",
             fontsize=10, color="#F9FAFB", y=1.01)
...
fig.patch.set_facecolor("#111827")
fig.suptitle("Pilgrimage / Religious Traveler — Segment Deep Dive\n"
...
ax1.bar(bm.index, bm.values, color="#F97316", alpha=.85, edgecolor="none")
...
fig = plt.figure(figsize=(14, 5))
fig.patch.set_facecolor("#111827")
fig.suptitle("Family Vacation Traveler — Segment Deep Dive\n"
             f"(Proxy: PAX 3–5, n={len(fam):,})",
             fontsize=10, color="#F9FAFB", y=1.01)
gs = gridspec.GridSpec(1, 3, figure=fig, wspace=0.35)

ax1 = fig.add_subplot(gs[0])
bm = fam["booking_month"].value_counts().reindex(range(1,13), fill_value=0)
ax1.bar(bm.index, bm.values, color="#06B6D4", alpha=.85, edgecolor="none")
ax1.set_title("Booking Month", fontsize=9, color="#06B6D4")
ax1.set_xticks(range(1,13))
ax1.set_xticklabels(["J","F","M","A","M","J","J","A","S","O","N","D"], fontsize=7)
ax1.set_ylabel("PNR Count")
ax1.axvspan(10.5,12.5, alpha=.12, color="#06B6D4", label="Christmas season")
ax1.legend(fontsize=7, facecolor="#1F2937", edgecolor="#374151", labelcolor="#F9FAFB")

ax2 = fig.add_subplot(gs[1])
reg = fam["Region"].value_counts().head(7)
ax2.barh(reg.index[::-1], reg.values[::-1], color="#06B6D4", alpha=.85, edgecolor="none")
...
ax3.barh(ch.index[::-1], ch.values[::-1], color="#06B6D4", alpha=.85, edgecolor="none")
...
fig = plt.figure(figsize=(14, 5))
fig.patch.set_facecolor("#111827")
fig.suptitle("Digital Nomad / Remote Worker — Segment Deep Dive\n"
...
ax1.bar(bm.index, bm.values, color="#A78BFA", alpha=.85, edgecolor="none")
...
ax2.barh(top_s.index[::-1], top_s.values[::-1], color="#A78BFA", alpha=.85, edgecolor="none")
ax2.set_title("Top ASEAN O&D Sectors", fontsize=9, color="#A78BFA")
ax2.set_xlabel("PNR Count")

ax3 = fig.add_subplot(gs[2])
lt = dn["lead_time"].dropna().clip(0, 120)
ax3.hist(lt, bins=30, color="#A78BFA", alpha=.85, edgecolor="none")
...
ax3.set_xlabel("Days Before Departure"); ax3.set_ylabel("Count")
...
for seg in SEG_ORDER:
    ...
    seg_radar[seg] = [
        min(fare_n, 1), min(lead_n, 1), min(grp_n, 1), intl_n, flex_n
    ]
...
fig.patch.set_facecolor("#111827")
fig.suptitle("Segment Fingerprints — 5-Dimension Radar\n"
...
for idx, (seg, values) in enumerate(seg_radar.items()):
    ...
    ax.set_xticklabels(["Fare","Lead","Group","Intl","Flex"],
...
fig.patch.set_facecolor("#111827")
fig.suptitle(f"Unassigned Records Analysis  (n={len(unassigned):,}, {len(unassigned)/len(df)*100:.1f}% of dataset)\n"
             "These records require additional features (Loyalty, Length of Stay) to classify",
             fontsize=10, color="#F9FAFB", y=1.02)
...
for bar in axes[0].patches:
    axes[0].text(bar.get_width()+10, bar.get_y()+bar.get_height()/2,
                 f"{bar.get_width():,}", va="center", fontsize=7, color="#9CA3AF")
...
axes[2].hist(lt, bins=40, color="#374151", edgecolor="none", alpha=.9)
...
axes[2].set_xlabel("Days Before Departure"); axes[2].set_ylabel("Count")
...
print(f"New files: 23 through 35")
