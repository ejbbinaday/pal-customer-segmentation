...
![The honest upper bound on any clustering claim here.](../reports/study_guide/ms_fig1_separation_ceiling.png)
...
![The methods do not agree with each other.](../reports/study_guide/ms_fig2_cross_method_ari.png)
...
### 2.1 And we tried six ways to make it go away

*Added 13 August 2026. Every threshold was written down before running — details in
`docs/continuum-levers-plan.md`.*

The obvious challenge to a null result is *"you didn't look hard enough."* So we ran six pre-registered
attempts to find structure the original setup might have missed:

**All silhouette rows use the identical metric** — best **Gower silhouette** across the same 3-method panel
(GMM · KMeans · LCA), k = 3–8, scored on a 3,000-row subsample. What changes between rows is the
*population*, which is the point of each lever. Lever G is the exception and is marked as such.

| We tried | Metric · population | Baseline | Its score | Bar to clear | Pass / Fail |
|---|---|---:|---:|---:|---|
| Adding **length of stay** as a new input | Gower silhouette · round-trip bookings | 0.323 | 0.319 | 0.45 | ❌ **Fail** |
| Removing **sea crew and bulk buyers** | Gower silhouette · all bookings, those excluded | 0.294 | 0.272 | 0.374 *(+0.08)* | ❌ **Fail** |
| Clustering **each route separately** | Gower silhouette · one O&D market at a time | 0.319 | 0.511 *(best of 6)* | 0.45 in a **majority** | ❌ **Fail** |
| Letting the model **build its own representation** | Gower silhouette · all bookings *(fitted in embedded space, **scored in the original** — so it stays comparable)* | 0.294 | 0.304 | 0.50 | ❌ **Fail** |
| Segmenting on **behaviour over time** | Gower silhouette · **customer grain**, repeat cohort | 0.319 | 0.211 | 0.50 | ❌ **Fail** |
| ⚠️ **Merging to 4 broad segments** | **Different metric** — *signal retained* (AUC ratio predicting `flown_any`) · all bookings | 0.348 | 0.194 | ≥ 0.328 | ❌ **Fail** |
| *— reference —* **scrambled data** | Gower silhouette · all bookings, every column permuted | — | **0.205** | — | *the floor* |
| *— reference —* **published ceiling** | Gower silhouette · **10-method panel**, not 3 | — | **0.381** | — | *the ceiling* |

**~14 pre-registered comparisons. None cleared.**

#### The number that makes all of this interpretable: a noise floor of 0.205

We scrambled every feature so no real structure could survive, then re-ran the whole panel. **It still
scored 0.205.** That is what *"nothing there"* looks like on this data — and it had never been measured.

Three things follow, and the second is the one to have ready:

1. **The 0.381 ceiling sits +0.176 above the floor — it is comfortably real.** The continuum finding
   stands, and is now properly calibrated rather than compared to a textbook zero.
2. **The standard bands are wrong for this data.** *">0.5 strong · 0.25–0.5 weak but real · <0.25 none"*
   assumes a floor of zero. **Here, "none" is 0.205** — so a score of 0.25 on this data is noise.
3. ⚠️ **The LCA sub-type silhouettes (0.204–0.264) sit at that floor.** A matched within-parent control is
   needed before acting on it, but the `provisional` flag on those sub-types now looks generous.

> **What this buys you in the room.** Yesterday the claim was *"ten clustering methods agreed there are no
> natural segments."* Today it is: ***"we then tried six different ways to overturn that — more features, a
> cleaner population, a learned representation, a coarser taxonomy, per-market, and behaviour over time —
> with every threshold fixed in advance. None cleared. And we measured what pure noise scores on this data,
> so we know our numbers are above it."*** That is a materially harder position to attack.
...
![Volume and value are inverted — the commercial case in one picture.](../reports/study_guide/eda_01_segments.png)
...
![What the machine learning actually contributes.](../reports/study_guide/sub_01_subtypes.png)
...
![V1 — do the segments differ on evidence the rules never saw?](../reports/study_guide/ms_fig4_construct_auc.png)
...
![V3 — the smoke-alarm test, and where the alarm stops working.](../reports/study_guide/ms_fig5_detection_floor.png)
