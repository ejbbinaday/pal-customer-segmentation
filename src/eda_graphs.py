...
for bar, val in zip(bars, null_pct.values):
    if val > 0:
        ax.text(
            val + 0.5, bar.get_y() + bar.get_height() / 2, f"{val:.1f}%", va="center", fontsize=8
        )
patches = [
...
bars = ax.bar(
    cabin_data.index,
    cabin_data.values,
    color=[PAL_LIGHT, PAL_GOLD, PAL_BLUE],
    edgecolor="white",
    width=0.5,
)
...
axes[0].axvline(
    df["Average Fare"].mean(),
    color=PAL_RED,
    ls="--",
    label=f"Mean ${df['Average Fare'].mean():.0f}",
)
axes[0].axvline(
    df["Average Fare"].median(),
    color=PAL_GOLD,
    ls="--",
    label=f"Median ${df['Average Fare'].median():.0f}",
)
axes[0].legend()
...
df["Cabin Label"] = df["Cabin"].map(
    {"Y": "Economy (Y)", "W": "Premium Economy (W)", "J": "Business (J)"}
)
...
sns.boxplot(
    ...
    y="Average Fare",
    order=cabin_order,
    palette=[PAL_LIGHT, PAL_GOLD, PAL_BLUE],
    ax=ax,
    showfliers=False,
    width=0.45,
)
...
sns.boxplot(
    ...
    y="Average Fare",
    order=region_order,
    palette=PALETTE[: len(region_order)],
    ax=ax,
    showfliers=False,
    width=0.5,
)
...
axes[0].hist(
    df["lead_time"].dropna().clip(0, 200), bins=60, color=PAL_BLUE, edgecolor="white", alpha=0.85
)
axes[0].set_title("Lead Time Distribution (0–200 days)", fontweight="bold")
axes[0].set_xlabel("Days Before Departure")
axes[0].set_ylabel("Count")
axes[0].axvline(
    df["lead_time"].mean(), color=PAL_RED, ls="--", label=f"Mean {df['lead_time'].mean():.0f}d"
)
axes[0].axvline(
    df["lead_time"].median(),
    color=PAL_GOLD,
    ls="--",
    label=f"Median {df['lead_time'].median():.0f}d",
)
axes[0].legend()
...
axes[1].set_xlabel("Booking Window")
axes[1].set_ylabel("PNR Count")
for bar in bars:
    pct = bar.get_height() / len(df) * 100
    axes[1].text(
        bar.get_x() + bar.get_width() / 2,
...
fig, ax = plt.subplots(figsize=(8, 5))
bars = ax.bar(
    pax.index.astype(str).tolist()[:-1] + ["10+"],
    pax.values,
    color=[PAL_BLUE if i == 1 else PAL_LIGHT if i <= 4 else PAL_GOLD for i in pax.index],
    edgecolor="white",
)
...
colors_p = [list(SEGMENT_PALETTE.values())[i % len(SEGMENT_PALETTE)] for i in range(len(proxy_df))]
fig, ax = plt.subplots(figsize=(10, 6))
...
proxies_kde = {
    ...
    "Balikbayan (INT→DOM Beyond)": df.loc[
        df["Itinerary Type"] == "Beyonds (INT - DOM)", "lead_time"
    ].dropna(),
    "Budget Leisure (Eco SS)": df.loc[
        df["Farebrand"] == "Economy Supersaver", "lead_time"
    ].dropna(),
...
proxy_order = (
    df.groupby("segment_proxy")["fare_per_pax"].median().sort_values(ascending=False).index
)
...
sns.boxplot(
    ...
    y="fare_per_pax",
    order=proxy_order,
    palette=proxy_colors[: len(proxy_order)],
    ax=ax,
    showfliers=False,
    width=0.5,
)
...
ch_proxy = pd.crosstab(df["segment_proxy"], df["Ticketing Channel"].fillna("Unknown"))
ch_proxy_pct = ch_proxy.div(ch_proxy.sum(axis=1), axis=0) * 100
fig, ax = plt.subplots(figsize=(13, 6))
sns.heatmap(
    ch_proxy_pct.round(1),
    annot=True,
    fmt=".1f",
    cmap="YlOrRd",
    ax=ax,
...
fig, ax = plt.subplots(figsize=(9, 5))
bars = ax.bar(top_sectors.index, top_sectors.values, color=PALETTE * 3, edgecolor="white")
...
ax.barh(
    y, missing, left=avail, color=PAL_RED, edgecolor="white", height=0.6, label="Columns Missing"
)
ax.set_yticks(y)
ax.set_yticklabels(labels_r, fontsize=9)
ax.set_xticks([0, 1, 2, 3])
ax.set_xticklabels(["0", "1/3", "2/3", "3/3"])
...
for label, (impact, feasibility) in features.items():
    ...
    ax.annotate(
        label,
        (feasibility + jitter[0], impact + jitter[1]),
