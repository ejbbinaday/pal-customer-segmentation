...
Fills figure placeholders in `docs/manuscript/manuscript-ch4-draft.md` (v1.2 renumbered the
placeholders into citation order; file basenames keep their original numbers — the draft's
figure table is the mapping):
  ...
  ms_fig4_construct_auc.png        segment-distinguishability AUC matrix (per-pair adaptive anchors)
...
def fig4_construct_auc() -> None:
    ...
    fig.text(
        0.01,
        -0.02,
        "Bands (adaptive measure): <0.60 not distinguishable · 0.60-0.75 weakly · >0.75 clearly "
        f"distinct. Outlined: weakest cell ({weakest['segment_a']} vs {weakest['segment_b']}, "
        f"{weakest['auc_adaptive']:.3f}).\n"
        "Cells rest on per-pair anchor sets (leaking anchors withheld pair-by-pair) — compare "
        "within equal withheld-anchor groups, not as a league table.\n"
        "Negative control 0.494-0.513 (pass). Strict two-anchor matrix: "
        "outputs/validate_construct/summary.md par.2. Source: outputs/validate_construct/pairs.csv",
        fontsize=8,
        color=INK_FAINT,
        va="top",
    )
...
def fig6_temporal_stability() -> None:
    ...
    tvd_share = df["delta_pp"].abs().sum() / 2
    tvd_rev = df["rev_delta_pp"].abs().sum() / 2
    panels = [
        (
            "share_earlier_pct",
            "share_later_pct",
            "Share of bookings (%)",
            f"TVD {tvd_share:.1f} pp",  # 1 dp: CSV deltas are rounded, 2 dp would drift from the report
        ),
        (
            "rev_share_earlier_pct",
            "rev_share_later_pct",
            "Share of revenue (%)",
            f"TVD {tvd_rev:.1f} pp",
        ),
