...
segment (esp. `Leisure`, which is half the book), picks the sub-count by BIC, and profiles the sub-types.

Top-level segments stay intact; this only adds a `sub_segment` descriptor within each parent.

⚠️ **Parents track the shipped taxonomy.** Re-pointed to waterfall v2 on 18 Aug 2026: the previous list
named `Budget/Adventure` (renamed `Leisure`) and `Last-Minute` (retired to a booking flag), so every
sub-type it produced was keyed on a segment that no longer exists.
...
# The five largest segments the v2 waterfall emits, excluding `Unassigned` (a residual has no
# sub-types worth naming until a rule claims it). Sizes at 18 Aug 2026: 11.60M / 3.91M / 2.87M /
# 2.18M / 1.17M bookings.
PARENTS = [
    "Leisure",
    "OFW/Migrant",
    "Balikbayan/VFR",
...
# Take the first `SAMPLE` rows of a hash-ordered scan. `hash()` is a pseudorandom permutation of the
# key, so this is a uniform sample, and it is exact and reproducible.
# ⚠️ Do NOT go back to `USING SAMPLE n ROWS (reservoir, seed)`: under `PRAGMA threads>1` DuckDB
# reservoir-samples per morsel and returns far fewer rows than asked for — measured 18 Aug 2026 at
# 20,191/40,000 for Leisure and **2,077/40,000 for Corporate**, a 19x shortfall that silently made the
# smallest parent the thinnest-sampled one. Deterministic, but wrong.
...
def main() -> None:
    ...
    lines = [
        ...
        "Top-level segments unchanged; sub-types below are the actionable refinement. "
        "Parents are the five largest segments the **v2** waterfall emits; "
        f"**{SAMPLE:,}** bookings sampled per parent (hash-ordered, reproducible).\n",
    ]
