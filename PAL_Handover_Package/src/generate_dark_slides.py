...
OUT_DIR = ROOT / "reports" / "poc_output" / "dark"
...
total_rev_risk = sum(REV_LOSS.get(row["proxy_segment"], 0) for _, row in misclassified.iterrows())
...
bars = ax.bar(
    range(len(seg_names_r)), recall_vals, color=bar_colors, edgecolor=BG, linewidth=0.8, width=0.65
)

# NFR threshold line
ax.axhline(0.91, color=RED, linestyle="--", linewidth=1.5, label="NFR-01 target  91%", alpha=0.85)

# Annotate each bar
for bar, r, pen, _seg in zip(bars, recall_vals, penalties, seg_names_r):
    star = " ★" if pen >= 5 else ""
    label = f"{r:.0%}\n×{pen}{star}"
    ax.text(
...
ax.set_xticklabels(
    seg_names_r, rotation=30, ha="right", fontsize=8.5, color=WHITE, fontfamily="DejaVu Sans"
)
...
for seg in SEGMENTS:
    ...
    ax.scatter(
        cx, cy, c=SEG_COLORS[seg], s=90, marker="D", edgecolors=WHITE, linewidths=0.8, zorder=5
    )
    ax.text(
        cx,
        cy + 0.18,
