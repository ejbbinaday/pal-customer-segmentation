...
K_RANGE = range(
    1, 10
)  # from 1: the manuscript's continuum claim needs BIC(k=1) and BIC(k=2) on the figure
...
def fig_route_region_revenue(con) -> None:
    # Companion to eda_02: same regions, same ordering (by bookings), with revenue share
    # beside bookings share so the volume-vs-value inversion is visible per region.
    # Revenue is booking-grain rev_pos, the same field eda_01's value panel uses.
    df = con.execute(f"""
        SELECT coalesce(dest_region,'Philippines (domestic)') region,
               count(*) n, sum(rev_pos) rev
        FROM read_parquet('{BOOKING}') GROUP BY 1 ORDER BY n DESC
    """).fetchdf()
    df["pct_bk"] = 100 * df["n"] / df["n"].sum()
    df["pct_rev"] = 100 * df["rev"] / df["rev"].sum()
    df["avg_rev"] = df["rev"] / df["n"]
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(
        [f"{r}\n(${a:,.0f}/bk)" for r, a in zip(df["region"], df["avg_rev"])],
        rotation=0 if len(df) <= 4 else 25,
        ha="right",
        fontsize=8.5,
    )
    ax.set_ylabel("Share (%)")
    ax.set_title(
        "Domestic carries the bookings, North America carries the revenue",
        fontweight="bold",
        loc="left",
        fontsize=12,
    )
    for xi, p in zip(x, df["pct_bk"]):
        ax.text(xi - w / 2, p + 0.6, f"{p:.0f}%", ha="center", fontsize=8.5, color=INK_SOFT)
    for xi, p in zip(x, df["pct_rev"]):
        ax.text(xi + w / 2, p + 0.6, f"{p:.0f}%", ha="center", fontsize=8.5, color=INK_SOFT)
    ax.set_ylim(0, max(df["pct_bk"].max(), df["pct_rev"].max()) * 1.15)
    ax.legend(frameon=False, fontsize=9, loc="upper right")
    save(fig, "eda_02b_region_revenue.png")
...
def fig_lead_and_value(con) -> None:
    ...
    ax1.text(4, ax1.get_ylim()[1] * 0.9, "is_last_minute flag ≤3d", color=SIGNAL, fontsize=9)
...
def fig_lca_bic(bic_tbl: pd.DataFrame) -> None:
    ...
    ari_max = bic_tbl["ARI_vs_proxy"].max()
    ax2.set_title(f"Only moderate agreement (ARI ≤ {ari_max:.2f})", fontweight="bold", loc="left")
    fig.suptitle(
...
def fig_pca(df: pd.DataFrame, labels: np.ndarray, k_star: int) -> None:
    ...
    for ax in (ax1, ax2):
        ...
        ax.grid(False)
    fig.suptitle(
        # `USING SAMPLE ... (reservoir)` is uniform, not stratified. Deliberately so: balancing by
        # segment or region would equalise group sizes and can manufacture apparent structure.
        "Preliminary clusters (60k uniform random sample, PCA projection)",
        fontsize=12.5,
        fontweight="bold",
        color=INK,
        x=0.01,
...
def _load_population_profiles() -> dict[str, "pd.DataFrame"]:
    """Parse outputs/sub_segments/population_profiles.md — the population-exact level-2
    profiles. Read, not refit: refitting here would create a second set of sub-type numbers
    that could disagree with the shipped assignment (subsegment_assign.py)."""
    text = (ROOT / "outputs" / "sub_segments" / "population_profiles.md").read_text()
    profiles: dict[str, pd.DataFrame] = {}
    parent, rows = None, []
    for line in text.splitlines():
        if line.startswith("## "):
            if parent and rows:
                profiles[parent] = pd.DataFrame(rows)
            parent, rows = line[3:].split(" — ")[0].strip(), []
        elif (
            parent and line.startswith("|") and not line.startswith("|:") and "sub_name" not in line
        ):
                ...
                rows.append(
                    {
                        "sub_name": cells[0],
                        "pct": float(cells[2]),
                        "med_lead": float(cells[3]),
                        "med_rev": float(cells[5]),
                    }
                )
    if parent and rows:
        profiles[parent] = pd.DataFrame(rows)
...
def fig_sub_segments() -> None:
    """Level-2 sub-types inside the five largest v2 segments, from the population-exact
    profiles (no refit)."""
    profiles = _load_population_profiles()
    fig, axes = plt.subplots(2, 3, figsize=(13.6, 7.6))
    axes = axes.ravel()
    for ax in axes[len(profiles) :]:
        ax.set_visible(False)
    for i, (seg, prof) in enumerate(profiles.items()):
        ax = axes[i]
        ...
        ax.set_yticks(y)
        ax.set_yticklabels(prof["sub_name"], fontsize=7.5)
        ax.set_xlim(0, prof["pct"].max() * 1.34)
        ax.set_xlabel("Share of segment (%)", fontsize=8.5)
        ax.tick_params(axis="x", labelsize=8)
        ax.set_title(f"{seg}  →  {len(prof)} sub-types", fontweight="bold", loc="left", fontsize=10)
        ax.grid(True, axis="x")
        ax.grid(False, axis="y")
        for yi, (pct, rev, lead) in enumerate(
            zip(prof["pct"], prof["med_rev"], prof["med_lead"], strict=True)
        ):
            ax.text(
                pct + prof["pct"].max() * 0.03,
                yi,
                f"${rev:,.0f} · {int(lead)}d",
                va="center",
                fontsize=7.5,
    ...
    fig.suptitle(
        "Sub-segmentation — LCA sub-types inside the five largest segments (population-exact)",
        fontsize=12.5,
        fontweight="bold",
        color=INK,
        x=0.01,
    ...
    fig.text(
        0.01,
        -0.015,  # below the axes; bbox_inches='tight' picks it up without colliding with xlabels
        "Bar label = direction · timing · value tier (· connectivity where names collide);  "
        "annotation = median revenue · median lead time.\n"
        "Fitted on count-weighted cell tables covering all 21.7M bookings in the five parents.  "
        "Source: outputs/sub_segments/population_profiles.md",
        fontsize=7.5,
        color=INK_FAINT,
        family="monospace",
    )
...
def main() -> None:
    ...
    # persist the exact table beside the figure — quoted by the manuscript and its plates
    bic_tbl.round({"BIC": 0, "ARI_vs_proxy": 3}).to_csv(
        FIGS.parent / "lca_bic_ari.csv", index=False
    )
