...
OUTPUT_DIR = ROOT / "reports" / "poc_output"
...
df = pd.read_csv(ROOT / "data" / "raw" / "synthetic_flight_passenger_data.csv")
...
rules = [
    ...
    ("Last-Minute", (df["lead_time"] <= 3) | (df["Travel_Purpose"] == "Emergency")),
    (
        "Digital Nomad",
        (df["Travel_Purpose"] == "Leisure")
...
print(
    f"  Pre-NL: {(df['proxy_segment'] != 'Unassigned').sum():,} labelled "
...
ax.set_title(
    "PAL Customer Segment Distribution\n(ML-Assigned — Synthetic PoC Data)",
    fontsize=12,
    fontweight="bold",
)
...
for seg in SEGMENTS:
    ...
    if mask.sum() == 0:
        continue
    ax.scatter(
        X_pca[mask, 0], X_pca[mask, 1], c=SEG_COLORS[seg], label=seg, alpha=0.35, s=10, linewidths=0
    )
...
ax.set_title(
    "Segment Cluster Separation — Penalty-Weighted PCA\n(Synthetic PoC Data)",
    fontsize=12,
    fontweight="bold",
)
...
for i, true_seg in enumerate(pen_segs):
    for j, _pred_seg in enumerate(pen_segs):
        if i != j:
...
ax.set_title(
    "Asymmetric Cost Matrix — Misclassification Penalty Weights\n"
...
for seg in target_segs:
    ...
    uplift_data[seg] = {
        sc: n_seg * REV_LOSS[seg] * pct_improve for sc, pct_improve in scenarios.items()
    }
...
for k, (sc, clr) in enumerate(zip(scenarios, sc_colors)):
    ...
    bars = ax.bar(
        x + k * width, vals, width, label=sc.replace("\n", " "), color=clr, edgecolor="white"
    )
    for bar, v in zip(bars, vals):
        ax.text(
            bar.get_x() + bar.get_width() / 2,
...
ax.set_title(
    "Projected Revenue Uplift from Reduced Misclassification\n"
...
ax.set_ylim(0, 1.12)
ax.set_title(
    "Per-Segment Recall — Asymmetric Validation\n"
...
for k, seg in enumerate(profile_segs):
    mask = df["model_segment"] == seg
    vals = []
    for _feat_label, (col, lo, hi) in profile_feats.items():
        raw = df.loc[mask, col].mean()
        vals.append((raw - lo) / (hi - lo))  # normalize 0–1
    offset = (k - len(profile_segs) / 2) * width + width / 2
    bars = ax.bar(
        x + offset, vals, width, label=seg, color=SEG_COLORS[seg], edgecolor="white", alpha=0.88
    )
...
ax.set_ylim(0, 1.15)
ax.set_title(
    "Segment Behavior Profiles — Key Discriminating Features\n(Normalized to 0–1 for comparison)",
    fontsize=11,
    fontweight="bold",
)
...
vp = ax.violinplot(lt_data, positions=range(len(top_segs)), showmedians=True, showextrema=False)
...
ax.set_title(
    "Booking Lead Time Distribution by Segment\n"
    "White line = median | Width = density of passengers",
    fontsize=11,
    fontweight="bold",
)

for i, (_seg, vals) in enumerate(zip(top_segs, lt_data)):
    ax.text(
        i,
        np.median(vals) + 2,
        f"{np.median(vals):.0f}d",
        ha="center",
        va="bottom",
        fontsize=8,
        fontweight="bold",
...
# SUMMARY
# ══════════════════════════════════════════════════════════════════════════════
total_rev_risk = sum((df["model_segment"] == seg).sum() * REV_LOSS[seg] * 0.01 for seg in SEGMENTS)
