...
# Prototype track: sample-features.csv carries pre-v2 labels, so plot the pre-v2 list.
from pal_colors import SEG_COLORS
from pal_colors import SEG_ORDER_V1 as SEG_ORDER

ROOT = Path(__file__).resolve().parents[1]
OUTPUT = ROOT / "outputs" / "hdbscan_output"
...
print(f"Proxy labels — labelled: {labeled_mask.sum():,}  unassigned: {(~labeled_mask).sum():,}")
...
top30_names = [
    feature_names[i]
    .replace("Ticketing Channel=", "Chan:")
...
bar_colors = [
    "#F59E0B" if v >= np.percentile(feature_weights, 90) else "#3B82F6" for v in top30_vals
]

fig, ax = plt.subplots(figsize=(14, 7), facecolor=BG)
bars = ax.barh(range(len(top30_vals)), top30_vals[::-1], color=bar_colors[::-1], height=0.65)
...
ax.set_title(
    "Penalty-Weighted Feature Scaling\n"
...
print(f"HDBSCAN result: {n_clusters} clusters  |  noise: {n_noise:,} ({noise_pct:.1f}%)")
...
ax.scatter(
    X_pca[plot_idx][micro, 0],
    X_pca[plot_idx][micro, 1],
    c="#4B5563",
    s=5,
    alpha=0.25,
...
ax.set_title(
    f"HDBSCAN  ·  Penalty-Weighted Feature Space  ·  {n_clusters} clusters\n"
...
fig.suptitle(
    f"HDBSCAN Cluster Size Distribution  ·  {n_clusters} clusters  (penalty-weighted features)",
    fontsize=11,
    color=TEXT,
    fontweight="bold",
)
...
ax1.bar(
    range(len(top30s)),
    top30s.values,
    color=[TAB_PAL[i % len(TAB_PAL)] for i in range(len(top30s))],
    width=0.7,
)
...
ax2.scatter(
    rank, sizes.values, c=[TAB_PAL[i % len(TAB_PAL)] for i in range(len(sizes))], s=15, alpha=0.75
)
ax2.set_xscale("log")
ax2.set_yscale("log")
...
seg_names_present = [s for s in SEG_ORDER if s in df["segment"].unique() and s != "Unassigned"]
seg_centroids = {}
...
cluster_to_seg = {cid: seg_names_present[nn_idx[i, 0]] for i, cid in enumerate(cluster_ids)}
...
dists = np.array(
    [np.linalg.norm(noise_X - seg_centroids[s], axis=1) for s in seg_names_present]
...
# ══════════════════════════════════════════════════════════════════════════════
seg_cluster_counts = (
    pd.Series(cluster_to_seg)
    .drop(-1, errors="ignore")
    .value_counts()
    .reindex(seg_names_present, fill_value=0)
)
seg_pnr_counts = df["hdb_segment"].value_counts().reindex(seg_names_present, fill_value=0)

fig, axes = plt.subplots(1, 2, figsize=(16, 6), facecolor=BG)
fig.suptitle(
    f"HDBSCAN {n_clusters} Micro-Clusters → 10 Segments  (nearest penalty-weighted centroid)",
    fontsize=11,
    color=TEXT,
    fontweight="bold",
)
...
ax1 = axes[0]
bars = ax1.bar(seg_names_present, seg_cluster_counts.values, color=colors_seg, width=0.65)
...
ax2 = axes[1]
bars2 = ax2.bar(seg_names_present, seg_pnr_counts.values, color=colors_seg, width=0.65)
for bar, val in zip(bars2, seg_pnr_counts.values):
    ax2.text(
        bar.get_x() + bar.get_width() / 2,
        bar.get_height() + max(seg_pnr_counts) * 0.01,
        f"{val:,}\n({val / len(df) * 100:.1f}%)",
        ha="center",
        va="bottom",
        fontsize=7,
...
ax2.set_title(
    f"PNRs per Segment  (noise={n_noise:,} auto-assigned to nearest centroid)",
    color=TEXT,
    fontsize=10,
    fontweight="bold",
)
...
fig.suptitle(
    "Final 10-Segment Assignment  ·  HDBSCAN + Penalty-Weighted Features  "
...
for ax, show_boundaries in zip(axes, [True, False]):
    ...
    if show_boundaries:
        ...
        ax.contour(xx, yy, Z, levels=levels, colors="white", linewidths=0.9, alpha=0.55, zorder=2)
...
patches = [mpatches.Patch(color=SEG_COLORS.get(s, "#60A5FA"), label=s) for s in seg_names_present]
fig.legend(
    handles=patches,
    fontsize=7.5,
    loc="lower center",
...
fig.suptitle(
    f"Noise Auto-Assignment  ·  {n_noise:,} Records Assigned to Nearest Segment Centroid",
    fontsize=11,
    color=TEXT,
    fontweight="bold",
)
...
bars = ax1.bar(noise_assign_series.index, noise_assign_series.values, color=colors_na, width=0.65)
...
for seg in noise_assign_series.index:
    ...
    ax2.scatter(
        np.full(len(sub_dist), seg), sub_dist, c=SEG_COLORS.get(seg, "#60A5FA"), s=6, alpha=0.4
    )
ax2.set_title(
...
ax3.set_title(
    "Auto-Assigned Noise in PCA Space\n(colour = assigned segment)",
    color=TEXT,
    fontsize=9,
    fontweight="bold",
)
...
# ══════════════════════════════════════════════════════════════════════════════
proxy_counts = (
    df["segment"].value_counts().reindex(seg_names_present + ["Unassigned"], fill_value=0)
)
hdb_counts = df["hdb_segment"].value_counts().reindex(seg_names_present, fill_value=0)
...
fig.suptitle(
    "Segment Distribution: Proxy Waterfall  vs  HDBSCAN Final (all records assigned)",
    fontsize=11,
    color=TEXT,
    fontweight="bold",
)
...
bars1 = ax.bar(
    x_proxy,
    [proxy_counts[s] for s in proxy_segs],
    width=0.65,
    color=cols_proxy,
    alpha=0.75,
    edgecolor=BORDER,
...
bars2 = ax.bar(
    x_hdb,
    [hdb_counts[s] for s in seg_names_present],
    width=0.65,
    color=cols_hdb,
    alpha=0.95,
    edgecolor="white",
...
ax.set_title(
    "HDBSCAN Final  (0 Unassigned — noise auto-assigned)",
    color=TEXT,
    fontsize=10,
    fontweight="bold",
)
...
ax.set_title(
    "Segment Centroids  ·  HDBSCAN + Penalty-Weighted Features  "
