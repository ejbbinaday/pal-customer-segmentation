...
def verdict(summary, drift, ctl, prof, transfer, censor, fg=None) -> list[str]:
    ...
    if len(transfer):
        # Ratio, not raw ARI: the raw number is dominated by how reproducible the method is at all,
        # and the highest ARI can belong to a method that actually lost more to time.
        #
        # ⚠️ Report the PANEL, never its best cell. Taking `sort_values(...).iloc[0]` here is a
        # selection effect — it is how the 29 Jul run published "ratio 1.13" on LCA and the 18 Aug
        # re-run silently switched the headline to GMM without anyone choosing. Same rule as V3's
        # detection floors: the verdict is what a **majority of the panel** agrees on, with the
        # spread quoted beside it. See knowledge base §15, 2026-08-18.
        ranked = transfer.sort_values("ratio_transfer_over_control", ascending=False)
        ratios = ranked["ratio_transfer_over_control"].astype(float)
        n_hold = int((ratios >= 0.95).sum())
        holds = n_hold > len(ratios) / 2
        split = not holds and n_hold > 0
        hi, lo = ranked.iloc[0], ranked.iloc[-1]
        panel = " · ".join(
            f"`{r['method']}` {r['ratio_transfer_over_control']}" for _, r in ranked.iterrows()
        )
        lines.append(
            (
                "5. **A model fitted a year earlier carves the later data essentially as well as one "
                "fitted on it** — on a majority of the panel. "
                if holds
                else "5. **The panel disagrees on whether a year-old model still carves the data.** "
                if split
                else "5. **Time costs the model something measurable on every method tested.** "
            )
            + f"Ratio of transfer over its own within-window ceiling, all {len(ratios)} methods: "
            f"{panel} — **{n_hold} of {len(ratios)} at or above 0.95**. The control is the "
            "**ceiling, not a baseline**: it is the same method disagreeing with *itself* across two "
            "halves of one period, no time involved. So the shortfall below it — not the raw ARI — is "
            "what a year costs. "
            + (
                f"Quote the panel, not one method: `{hi['method']}` reaches "
                f"{hi['ratio_transfer_over_control']} while `{lo['method']}` only reaches "
                f"{lo['ratio_transfer_over_control']}, so any refit-cadence claim must name its "
                "method — and none should be made at all until this stage runs across several seeds "
                "(single-seed point estimates with no interval are what produced the withdrawn "
                "1.13). "
                if split
                else ""
            )
            + "Note the raw ARIs are well under 1.0 for *both* windows — that is the continuum showing "
            "up again (these methods do not reproduce themselves exactly on any split), which is why "
            "the ratio is the readable number."
        )
