...
def _scatter_all(ax, alpha_lab=0.55, alpha_unlab=0.15, s_lab=12, s_unlab=6):
    ...
    ax.scatter(unlab["pc1"], unlab["pc2"],
               c=SEG_COLORS["Unassigned"], s=s_unlab, alpha=alpha_unlab, zorder=2)
    # labelled
    for seg in seg_order:
        sub = labeled[labeled["segment"] == seg]
        ax.scatter(sub["pc1"], sub["pc2"],
                   c=SEG_COLORS[seg], s=s_lab, alpha=alpha_lab,
                   zorder=3, label=f"{seg} ({len(sub):,})")

def _legend(ax, ncol=2, fontsize=7.5):
    patches = [mpatches.Patch(color=SEG_COLORS[s], label=s) for s in seg_order]
    ax.legend(handles=patches, fontsize=fontsize, ncol=ncol,
...
cl = ax.contour(xx, yy, Z_num, levels=levels,
                colors="white", linewidths=0.9, alpha=0.55, zorder=2)

_scatter_all(ax)
_legend(ax, ncol=2)
_axis_labels(ax)
ax.set_title("PCA-2D  ·  Decision Boundaries  ·  KNN-15 classifier on proxy labels\n"
...
_legend(ax, ncol=2)
_axis_labels(ax)
ax.set_title("PCA-2D  ·  Decision Boundary Lines Only  ·  KNN-15\n"
...
fig, axes = plt.subplots(nrows, ncols,
                         ...
                         facecolor=BG)
fig.suptitle("Per-Segment Zoom  ·  Decision Boundaries  ·  KNN-15 (PCA-2D)",
...
for i, seg in enumerate(seg_order):
    ...
    zx1 = max(zx1, x1_min);  zx2 = min(zx2, x1_max)
    zy1 = max(zy1, x2_min);  zy2 = min(zy2, x2_max)

    # fine mesh for this window
    xx_z, yy_z = np.meshgrid(np.arange(zx1, zx2, ZOOM_STEP),
                              np.arange(zy1, zy2, ZOOM_STEP))
    ...
    ax.contourf(xx_z, yy_z, Z_z, levels=levels,
                cmap=cmap_fill, alpha=0.25, zorder=1)

    # boundary lines — thicker inside zoom panels
    ax.contour(xx_z, yy_z, Z_z, levels=levels,
               colors="white", linewidths=1.1, alpha=0.70, zorder=2)
    ...
    ax.scatter(others["pc1"], others["pc2"],
               c="#374151", s=8, alpha=0.35, zorder=3)
    ...
    ax.scatter(unlab_win["pc1"], unlab_win["pc2"],
               c="#4B5563", s=6, alpha=0.25, zorder=3)

    # focal segment (bright, on top)
    ax.scatter(sub["pc1"], sub["pc2"],
               c=SEG_COLORS[seg], s=18, alpha=0.75,
    ...
    ax.set_title(f"{seg}\n(n={len(sub):,})",
                 fontsize=9, color=SEG_COLORS[seg], fontweight="bold", pad=6)
...
ax.contour(xx, yy, Z_num, levels=levels,
           colors="white", linewidths=0.7, alpha=0.45, zorder=2)
...
for seg in seg_order:
    ...
    rect = mpatches.FancyBboxPatch(
        ...
        linewidth=1.8, edgecolor=SEG_COLORS[seg],
        facecolor="none", zorder=6, alpha=0.9
    )
    ax.add_patch(rect)

    # label near top-right corner of box
    ax.text(zx2, zy2 + 0.05, seg,
            fontsize=6.5, color=SEG_COLORS[seg], fontweight="bold",
            ha="right", va="bottom", zorder=7,
            bbox=dict(fc=BG, ec="none", alpha=0.6, pad=1))

_legend(ax, ncol=2)
_axis_labels(ax)
ax.set_title("PCA-2D  ·  Zoom Regions per Segment  (coloured rectangles)\n"
