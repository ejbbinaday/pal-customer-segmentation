...
# ── Route config ───────────────────────────────────────────────────────────────
PH_AIRPORTS = {
    "DVO","CEB","ILO","BCD","ZAM","GEN","MNL","KLO",
    "PPS","TUG","LAO","CRM","CYZ","CGY","MPH","BXU","TAG",
}
DOM_SECTORS = [
    "DVOMNL","MNLDVO","CEBMNL","MNLCEB","ILOMNL","MNLILO",
    "BCDMNL","MNLBCD","ZAMMNL","MNLZAM","GENMNL","MNLGEN",
    "CEBDVO","DVOCEB","CEBILO","ILOCEB","CEBPPS","PPSCEB",
]
INT_SECTORS = [
    "MNLBKK","BKKMNL","MNLSYD","SYDMNL","MNLLAX","LAXMNL",
    "MNLHKG","HKGMNL","MNLNRT","NRTMNL","MNLICN","ICNMNL",
    "MNLSIN","SINMNL","MNLDXB","DXBMNL","MNLKUL","KULMNL",
]
BASE_PAX = {
    "DVOMNL":5200,"MNLDVO":4800,"CEBMNL":8500,"MNLCEB":8200,
    "ILOMNL":3200,"MNLILO":3000,"BCDMNL":3500,"MNLBCD":3300,
    "ZAMMNL":2100,"MNLZAM":2000,"GENMNL":1800,"MNLGEN":1700,
    "CEBDVO":2400,"DVOCEB":2300,"CEBILO":1600,"ILOCEB":1500,
    "CEBPPS":1200,"PPSCEB":1100,
    "MNLBKK":4500,"BKKMNL":4300,"MNLSYD":3200,"SYDMNL":3100,
    "MNLLAX":2800,"LAXMNL":2700,"MNLHKG":5500,"HKGMNL":5300,
    "MNLNRT":4100,"NRTMNL":4000,"MNLICN":3800,"ICNMNL":3700,
    "MNLSIN":5200,"SINMNL":5000,"MNLDXB":3600,"DXBMNL":3500,
    "MNLKUL":4200,"KULMNL":4000,
}
...
SEASONALITY = {1:0.85,2:0.80,3:0.90,4:0.95,5:1.00,6:1.05,
               7:1.15,8:1.20,9:0.90,10:0.85,11:1.00,12:1.25}
YOY_RANGE = {
    "Leisure":  (-0.05,  0.05),
    "VFR":      ( 0.10,  0.20),
    "Business": ( 0.00,  0.05),
...
# ── Fare config ────────────────────────────────────────────────────────────────
BASE_FARE = {
    ("dom","Y"):  3_500, ("dom","W"):  8_500, ("dom","J"): 18_000,
    ("int","Y"): 12_000, ("int","W"): 28_000, ("int","J"): 65_000,
}
SEG_FARE_MULT = {
    "Business": 1.35, "VFR": 0.98, "Leisure": 0.90, "OFW": 0.87, "Labor": 0.80,
}
FARE_YOY_RANGE = {
    "Business": (0.05, 0.12),
    "VFR":      (0.02, 0.07),
...
# ── Capacity config ────────────────────────────────────────────────────────────
SEATS_PER_OP = {
    ("dom","Y"): 140, ("dom","W"): 16,  ("dom","J"): 8,
    ("int","Y"): 240, ("int","W"): 32,  ("int","J"): 24,
}
...
BOOKING_ADV_PCT = {
    "Business": 0.72, "VFR": 0.58, "Leisure": 0.48, "OFW": 0.42, "Labor": 0.35,
}
...
def _flight_base(sector: str) -> int:
    return int(hashlib.md5(sector.encode()).hexdigest()[:5], 16) % 600 + 100
...
def build_summary() -> pd.DataFrame:
    ...
    for sector in DOM_SECTORS + INT_SECTORS:
        ...
        base_fares = {
            ...
            for cb in CABIN_SPLIT for seg in SEGMENTS
        }
            ...
            for month in range(1, 13):
                ...
                for cabin_code, cab_share in CABIN_SPLIT.items():
                    ...
                    for seg in SEGMENTS:
                        ...
                        seg_pax = base_seg_pax if year == 2025 else max(
                            0, int(base_seg_pax * (1 + seg_growth[seg]) * rng.uniform(0.97, 1.03))
                        )
                        if seg_pax == 0:
                            continue
...
def _yoy_style(pct: float) -> tuple[str, str]:
    ...
    if pct >= 10:    return "#27ae60", "white"
    elif pct >= 1:   return "#f1c40f", "#333"
    elif pct > -1:   return "transparent", "#333"
    elif pct >= -10: return "#e8c547", "#333"
    elif pct >= -30: return "#e07b39", "white"
    elif pct >= -60: return "#e74c3c", "white"
    else:            return "#c0392b", "white"


def _lf_style(lf: float) -> tuple[str, str]:
    if lf >= 85:   return "#27ae60", "white"
...
def render_table(
    ...
    monthly: dict,
    year1: int, year2: int,
    month_label: str, month_num: int,
    show_vs_net: bool = True,
) -> str:
    def pct(v, t):    return round(v / t * 100) if t else 0
    def yoy(v1, v2):  return round((v2 - v1) / v1 * 100) if v1 else 0
    def fare_fmt(f):  return f"&#8369;{f:,.0f}"
    ...
    vs_net_col = """<th rowspan="2" class="grp" style="border-bottom:1px solid #c8d8e8">vs.<br>Network</th>""" if show_vs_net else ""
...
def _monthly_pax(filt_all: pd.DataFrame, year: int) -> dict:
    ...
    return {
        seg: [int(fy[fy["segment"] == seg].groupby("month")["pax"].sum().get(m, 0))
              for m in range(1, 13)]
        for seg in SEGMENTS
    }


def _load_factor(df: pd.DataFrame, cap: pd.DataFrame,
                 month_num: int, od_sel: str, flight_sel: str, cabin_sel: str) -> pd.DataFrame:
    def flt(frame):
        f = frame[frame["month"] == month_num]
        if od_sel     != "All": f = f[f["od"]         == od_sel]
        if flight_sel != "All": f = f[f["flight"]     == flight_sel]
        if cabin_sel  != "All": f = f[f["cabin_name"] == cabin_sel]
        return f
...
def _alerts(df: pd.DataFrame, month_num: int, cabin_sel: str, threshold: int) -> pd.DataFrame:
    ...
    return (comb[comb["yoy"].abs() >= threshold]
            .sort_values(["od", "segment"])
            .reset_index())
...
def main():
    ...
    with st.sidebar:
        ...
        alert_thr = st.slider(
            "Alert threshold (%)", 10, 50, 20, step=5,
            help=(
    ...
    if od_sel     != "All": filt = filt[filt["od"]         == od_sel]
    if flight_sel != "All": filt = filt[filt["flight"]     == flight_sel]
    if cabin_sel  != "All": filt = filt[filt["cabin_name"] == cabin_sel]

    # All months for sparklines & advance (same od/flight/cabin, all months)
    filt_all = df.copy()
    if od_sel     != "All": filt_all = filt_all[filt_all["od"]         == od_sel]
    if flight_sel != "All": filt_all = filt_all[filt_all["flight"]     == flight_sel]
    if cabin_sel  != "All": filt_all = filt_all[filt_all["cabin_name"] == cabin_sel]
    ...
    summary  = _pivot(filt)
    network  = _pivot(df[df["month"] == month_num] if cabin_sel == "All"
    ...
    with tab1:
        ...
        with cc1:
            with st.container(border=True):
                st.plotly_chart(
                    _hbar(summary["y1_pax"][summary["y1_pax"] > 0], f"Segment Mix · 2025-{month_sel}"),
                    use_container_width=True, config={"displayModeBar": False},
                )
        with cc2:
            with st.container(border=True):
                st.plotly_chart(
                    _hbar(summary["y2_pax"][summary["y2_pax"] > 0], f"Segment Mix · 2026-{month_sel}"),
                    use_container_width=True, config={"displayModeBar": False},
                )
